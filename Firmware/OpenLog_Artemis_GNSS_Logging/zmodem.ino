...
size_t DSERIALprint(const __FlashStringHelper *ifsh)
{
  ...
  while (1) {
    if (DSERIAL == nullptr) {
      Serial.println("DSERIAL is null!");
    }
//    else {
//      Serial.println("DSERIAL is defined!");
//    }
    unsigned char c = pgm_read_byte(p++);
    if (c == 0) break;
    if (DSERIAL->availableForWrite() > SERIAL_TX_BUFFER_SIZE / 2) {
      DSERIAL->flush();
    }
...
{
  //Serial.println(F("Dislplaying SD Card Help...\n"));
